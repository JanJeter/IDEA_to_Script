...
export class PremiseAgentService {
  ...
  async generate(input: {
    ...
    if (input.project.visitorId !== input.visitorId) {
      throw new Error('Agent visitor capability does not match the project owner');
    }
    ...
    const memory = new StaticProjectMemoryPort({
      ...
      canonical: {
        ...
        confirmedStage: null,
        premise:
          input.project.premise && input.project.synopsis && input.project.theme
            ? {
                title: input.project.title,
