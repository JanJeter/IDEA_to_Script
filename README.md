...
打开页面后：左侧点击「**载入示例**」→ 点击「**开始转换**」→ 右侧即出现 YAML 剧本，可复制 / 下载 / 校验。

### 前端交互说明

- 输入区会防抖调用 `/api/parse`，实时显示字数、章节数量和前 5 个章节标题。
- 不足 3 章时会在前端给出明确提示，避免无效转换请求。
- 转换过程使用 `/api/convert` 的 NDJSON 流式事件驱动五步进度，失败时保留可读错误。
- YAML 输出区支持复制反馈、下载和二次 Schema 校验，校验错误会直接列在预览下方。
- 页面在窄屏下切换为单列布局，便于比赛演示时用笔记本或平板展示。
